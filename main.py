        ...
        for line in lines[1:]:
            ...
            if len(parts) >= 6:
                ...
                disks[mount] = {
                    ...
                    "available": parts[3], "use_percent": parts[4].replace("%", ""),
                }
        return disks
    except Exception:
        return {}
...
async def container_stats(container_id: str):
    ...
    return {
        "id": container_id[:12], "cpu_percent": cpu_percent,
        ...
        "memory_percent": mem_percent, "network_rx": net_rx, "network_tx": net_tx,
    }
...
async def all_containers_stats():
    ...
    return {"containers": containers, "system": {"memory": system, "disk": disks}}
...
:root {
  ...
  --border: #30363d; --border-light: #21262d; --text: #e6edf3; --text-dim: #8b949e;
  --text-bright: #f0f6fc; --accent: #58a6ff; --accent-dim: #1f6feb;
  --green: #3fb950; --green-dim: #238636; --red: #f85149; --red-dim: #da3633;
  --yellow: #d29922; --yellow-dim: #9e6a03; --orange: #db6d28; --purple: #bc8cff;
  --cyan: #39d2c0; --shadow: rgba(0,0,0,0.4); --radius: 12px; --radius-sm: 8px;
  --radius-xs: 6px; --transition: 0.25s cubic-bezier(0.4, 0, 0.2, 1);
}
[data-theme="light"] {
  --bg: #f6f8fa; --bg2: #fff; --card: #fff; --card-hover: #f3f4f6;
  --border: #d0d7de; --border-light: #e6e8eb; --text: #1f2328; --text-dim: #656d76;
  --text-bright: #1f2328; --accent: #0969da; --accent-dim: #0550ae;
  --green: #1a7f37; --green-dim: #116329; --red: #cf222e; --red-dim: #a40e26;
  --yellow: #9a6700; --yellow-dim: #7d4e00; --orange: #bc4c00; --purple: #8250df;
  --cyan: #1b7c83; --shadow: rgba(0,0,0,0.08);
}
[data-theme="ocean"] {
  --bg: #0a1628; --bg2: #0d1f3c; --card: #112645; --card-hover: #163056;
  --border: #1c3a5f; --border-light: #152e4d; --text: #c3d4e6; --text-dim: #6b8cae;
  --text-bright: #e8f0fe; --accent: #38bdf8; --accent-dim: #0284c7;
  --green: #34d399; --green-dim: #059669; --red: #fb7185; --red-dim: #e11d48;
  --yellow: #fbbf24; --yellow-dim: #d97706; --orange: #fb923c; --purple: #a78bfa;
  --cyan: #22d3ee; --shadow: rgba(0,0,0,0.5);
}
[data-theme="purple"] {
  --bg: #13081f; --bg2: #1a0e2e; --card: #221440; --card-hover: #2a1a4d;
  --border: #3d2666; --border-light: #2e1c52; --text: #d8c8f0; --text-dim: #8b7aab;
  --text-bright: #f0e8ff; --accent: #c084fc; --accent-dim: #9333ea;
  --green: #4ade80; --green-dim: #16a34a; --red: #f87171; --red-dim: #dc2626;
  --yellow: #facc15; --yellow-dim: #ca8a04; --orange: #fb923c; --purple: #e879f9;
  --cyan: #22d3ee; --shadow: rgba(0,0,0,0.5);
}
...
.header h1 { font-size: 18px; font-weight: 700; display: flex; align-items: center; gap: 8px; letter-spacing: -0.3px; }
.header h1 .icon { font-size: 24px; }
.header-right { display: flex; align-items: center; gap: 10px; }
.refresh-btn {
  background: var(--accent-dim); color: #fff; border: none; padding: 8px 18px;
...
/* Theme Switcher */
.theme-switcher { display: flex; gap: 4px; background: var(--bg2); border-radius: var(--radius-sm); padding: 3px; border: 1px solid var(--border); }
.theme-btn { width: 28px; height: 28px; border-radius: 6px; border: 2px solid transparent; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; transition: all var(--transition); background: transparent; color: var(--text); }
...
.top-status .sys-card { padding: 12px 16px; }
.top-status .sys-card .value { font-size: 18px; }

/* ========== PORTS ========== */
.ports-bar {
  margin-bottom: 16px; padding: 16px 20px; background: var(--card);
  border: 1px solid var(--border); border-radius: var(--radius); box-shadow: 0 2px 8px var(--shadow);
}
.ports-bar-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.ports-bar-title { font-size: 14px; font-weight: 700; display: flex; align-items: center; gap: 8px; }
.ports-bar-count { font-size: 12px; color: var(--text-dim); background: var(--bg2); padding: 2px 10px; border-radius: 12px; border: 1px solid var(--border); }
.ports-list { display: flex; flex-direction: column; gap: 6px; }
.port-item {
  display: flex; align-items: center; gap: 8px; padding: 8px 12px;
  background: var(--bg2); border: 1px solid var(--border-light); border-radius: var(--radius-sm);
  font-size: 13px; font-weight: 600; font-family: 'SF Mono', 'Cascadia Code', 'Consolas', monospace;
  color: var(--text-bright); transition: all var(--transition);
}
.port-item:hover { border-color: var(--accent); background: var(--card-hover); transform: translateX(2px); box-shadow: 0 2px 8px var(--shadow); }
.port-item .port-num { color: var(--accent); font-size: 16px; font-weight: 700; min-width: 50px; text-align: right; }
.port-item .port-arrow { color: var(--text-dim); font-size: 12px; }
.port-item .port-container { color: var(--text-dim); font-size: 11px; font-weight: 400; margin-left: auto; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.port-item .port-proto { color: var(--yellow); font-size: 10px; font-weight: 500; }
.ports-empty { color: var(--text-dim); font-size: 13px; padding: 8px 0; }
...
.sys-card {
  ...
  padding: 16px 18px; transition: all var(--transition);
}
...
.cat-tab {
  ...
  padding: 8px 18px; border-radius: 24px; cursor: pointer; font-size: 13px; font-weight: 500;
  transition: all var(--transition); display: flex; align-items: center; gap: 6px;
}
...
.cat-tab .count { background: rgba(255,255,255,0.15); padding: 1px 7px; border-radius: 10px; font-size: 11px; font-weight: 600; }
.cat-tab.active .count { background: rgba(255,255,255,0.25); }

/* ========== SECTION HEADERS ========== */
.section-header { display: flex; align-items: center; gap: 10px; margin: 24px 0 14px; padding-bottom: 10px; border-bottom: 1px solid var(--border); }
...
.section-count { font-size: 12px; color: var(--text-dim); background: var(--bg2); padding: 2px 8px; border-radius: 10px; border: 1px solid var(--border); }

/* ========== CONTAINER ROWS ========== */
.container-rows { display: flex; flex-direction: column; gap: 6px; }
.container-row {
...
.container-row .row-status { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
...
.container-row .row-name { font-size: 14px; font-weight: 600; min-width: 140px; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--text-bright); }
.container-row .row-image { font-size: 11px; color: var(--text-dim); font-family: 'SF Mono', 'Cascadia Code', monospace; max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.container-row .row-ports { display: flex; gap: 4px; flex-wrap: wrap; flex: 1; min-width: 0; }
.container-row .row-port { font-size: 11px; font-family: 'SF Mono', 'Cascadia Code', monospace; padding: 2px 6px; background: rgba(88,166,255,0.08); color: var(--accent); border-radius: 4px; border: 1px solid rgba(88,166,255,0.15); }
.container-row .row-stats { display: flex; gap: 12px; font-size: 11px; color: var(--text-dim); min-width: 120px; justify-content: flex-end; }
.container-row .row-actions { display: flex; gap: 6px; flex-shrink: 0; }
.container-row .row-actions button { padding: 4px 10px; font-size: 11px; font-weight: 600; border-radius: 4px; cursor: pointer; background: transparent; border: 1px solid var(--border); color: var(--text); transition: all var(--transition); }
...
.toast {
  ...
  background: var(--card); border: 1px solid var(--border); border-radius: var(--radius-sm);
  box-shadow: 0 4px 16px var(--shadow); font-size: 14px; font-weight: 500;
  transform: translateY(100px); opacity: 0; transition: all 0.3s ease; z-index: 200;
}
...
.error-banner {
  ...
  border-radius: var(--radius-sm); padding: 12px 16px; margin-bottom: 16px; font-size: 13px; color: var(--red);
}
...
document.querySelectorAll('.theme-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    localStorage.setItem('docker-panel-theme', t);
  });
});
const saved = localStorage.getItem('docker-panel-theme');
if (saved) {
...
async function loadData() {
  ...
  try {
    ...
    if (!respAll.ok) { const err = await respAll.json(); throw new Error(err.detail || 'API error'); }
...
function renderTopStatus(sys) {
  ...
  for (const [mount, info] of Object.entries(disk)) {
    ...
    html += `<div class="sys-card">
      ...
      <div class="sub">可用 ${info.available} MB</div>
      <div class="bar"><div class="bar-fill" style="width:${pct}%;background:${color}"></div></div>
    </div>`;
  }
  ...
  document.getElementById('topStatus').innerHTML = html || '<div class="sys-card"><div class="label">系统信息</div><div class="value">加载中...</div></div>';
}

function renderCategories() {
  document.getElementById('count-all').textContent = allContainers.length;
  document.getElementById('count-running').textContent = allContainers.filter(c => c.state === 'running').length;
  document.getElementById('count-stopped').textContent = allContainers.filter(c => c.state !== 'running').length;
}
...
function renderContainers() {
  ...
  if (currentCategory === 'running') stopped = [];
  else if (currentCategory === 'stopped') running = [];
  let html = '';
  if (running.length) {
  ...
  if (!running.length && !stopped.length) {
    html = '<div style="text-align:center;padding:40px;color:var(--text-dim)">' + (currentCategory === 'all' ? '没有容器' : currentCategory === 'running' ? '没有运行中的容器' : '没有未使用的容器') + '</div>';
  }
  el.innerHTML = html;
}
...
function renderRow(c, i) {
  ...
  const statsHtml = (c.stats && isRunning)
    ? `<span>CPU ${c.stats.cpu_percent||0}%</span><span>MEM ${c.stats.memory_usage_mb||0}/${c.stats.memory_limit_mb||0}MB</span>`
    : '<span>-</span>';
...
async function doAction(id, action, btn) {
  ...
  try {
    ...
    if (!resp.ok) { const err = await resp.json(); throw new Error(err.detail || '操作失败'); }
